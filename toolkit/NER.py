...
def nowok(s):  # 当前词的词性筛选
    ...
    if s == 'n' or s == 'np' or s == 'ns' or s == 'ni' or s == 'nz' or s == 'uw':
        return True
...
def get_detail_explain(s):
    ...
    if s == 5:
        return '包括核物理名词，核物理学相关术语'
    if s == 6:
        return '包括化学名词，化学相关术语'
    if s == 7:
        return '核技术名称，核技术相关术语'
    if s == 8:
        return '核应用名称，核应用相关术语'
    if s == 9:
        return '法规名称，法规相关术语'
    if s == 10:
        return '核武器名称，核武器相关术语'
    if s == 11:
        return '与核安全领域没有特别直接的关系，但是也是实体'
    # if s == 10:
...
# from toolkit1.pre_load import pre_load_thu,neo_con
